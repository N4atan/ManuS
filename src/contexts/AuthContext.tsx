...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
    const [user, setUser] = React.useState<User | null>(() => {
        const storedUser = localStorage.getItem("@manus:user");
        return storedUser ? JSON.parse(storedUser) : null;
    });
    const [isAuthenticated, setIsAuthenticated] = React.useState(() => {
        return !!localStorage.getItem("@manus:user");
    });
    ...
    const login = async (email: string) => {
        try {
            if (isAuthenticated) {
                ...
                toast('Você já está logado', { icon: '⚠️' });
                return;
            }
            ...
            localStorage.setItem("@manus:user", JSON.stringify(tempUser));
    ...
    const logout = () => {
        ...
        localStorage.removeItem("@manus:user");
